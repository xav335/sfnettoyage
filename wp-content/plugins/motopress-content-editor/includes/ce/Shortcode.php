...
//        var_dump($wp_query->post->ID);
    ...
    public function register() {
        ...
        add_action('wp_ajax_motopress_posts_grid_load_more', array($this, 'motopressPostsGridLoadMore'));
        add_action('wp_ajax_nopriv_motopress_posts_grid_load_more', array($this, 'motopressPostsGridLoadMore'));
    }
    /**
...
//            $error = $motopressCELang->CEImageIdEmpty;
        ...
        if (empty($error)) {
            ...
            if ($caption === 'true') {
                $imgHtml .= '<figcaption class="image-caption">' . $attachment->post_excerpt . '</figcaption>';
            }
...
//        $splitStyle = self::splitStyleClasses($mp_style_classes);
        ...
        $buttonHtml = '<div class="motopress-button-obj ' . $alignClass . $classes . self::getMarginClasses($margin) . '"><a href="' . $link . '" class="' . self::getBasicClasses(self::PREFIX . 'button') . $mp_style_classes . $fullWidthClass . '"';
        ...
        if ($icon_position == 'left') {
            $buttonHtml .= $iconHTML . $text;
        } else {
            $buttonHtml .= $text . $iconHTML;
        }
...
//            'fullwidth' => 'true'
        ...
        if ($tabsMatched) {
            ...
            $tabDefaults = array(
                ...
                'icon_size' => 'normal',
                'icon_custom_size' => '26',
                'icon_color' => 'mp-text-color-black',
                'icon_custom_color' => '#000000',
...
//                            'terms' => $custom_tax_terms,
                        ...
                        $loadMoreScript = '<p class="motopress-hide-script">';
                            ...
                            ?>
                                <script type="text/javascript">
                                    function loadMorePostsFor<?php echo $gridId; ?>() {
                                        var data = {
                                        ...
                                        motoLoadMorePosts('<?php echo $gridId; ?>', data);
                                    }
                                </script>
                            <?php
    ...
    public function motopressPostsGridLoadMore() {
        $attrs = array(
            'pagination' => 'false',
            'load_more_btn' => 'true',
            'is_ajax' => true
        );
        $fields = array('query_type', 'post_type', 'columns', 'category', 'tag', 'posts_per_page', 'posts_order', 'custom_tax', 'custom_tax_field', 'custom_tax_terms', 'custom_query', 'ids', 'template', 'posts_gap', 'show_featured_image', 'image_size', 'image_custom_size', 'title_tag', 'show_date_comments', 'show_content', 'short_content_length', 'read_more_text', 'ajax_paged');
        foreach ($fields as $field) {
            if (isset($_POST[$field])) {
                $attrs[$field] = $_POST[$field];
            }
        }

        $responseHtml = $this->motopressPostsGrid($attrs);
    ...
    public function motopressServiceBox($attrs, $content = null){
        extract(shortcode_atts(self::addStyleAtts(array(
            ...
            'button_custom_bg_color' => '#ffffff',
            'button_custom_text_color' => '#000000',
            'button_align' => 'center'
